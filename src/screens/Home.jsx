...
const Home = memo(() => {
  ...
  return <View className="w-full h-full" style={{
    backgroundColor: color.bgColor2
  }}>
      <SearchSection />
      <View className="container mx-auto p-0.5 px-1 pb-0" style={{
      flex: 1
    }}>
        {loading ? <View className="flex flex-row items-center justify-center" style={{
        flex: 1
            ...
            {list.length > 0 ? <FlashList data={list} {...lastReadedSurah > -1 && {
          initialScrollIndex: lastReadedSurah
        }} renderItem={({
          item
        }) => {
          return <ShortSurah surah={item} />;
        }} estimatedItemSize={97} /> : <View className="flex flex-row items-center justify-center" style={{
          flex: 1
        }}>
                <Text className="text-lg font-semibold text-center" style={{
