...
import { SAVE_TO_READ_LATER, SET_ARABIC_TEXT_SIZE, SET_AYAH_DETAILS, SET_BANLGA_TEXT_SIZE, SET_DARK_MODE, SET_ENGLIS_TEXT_SIZE, SET_FAVOURITE_SURAH_LIST, SET_HAFEZI_FONT, SET_IS_SHOW_AUDIO_PLAYER, SET_IS_SHOW_BANGLA_TEXT, SET_IS_SHOW_COPY_BUTTON, SET_IS_SHOW_ENGLISH_TEXT, SET_IS_SHOW_SAVE_BUTTON, SET_IS_SHOW_TAFSIR, SET_LAST_READ_AYAH, SET_LAST_READ_SURAH, SET_TAJWEED } from "../store/constants/constant";
...
const Splash = memo(() => {
  ...
  const {
    loading
  } = useSelector(state => state.shortSurahList);
  ...
  useEffect(() => {
    const fn = async () => {
      try {
        ...
        let arabicTextSize = JSON.parse(await AsyncStorage.getItem("arabicTextSize"));
        if (arabicTextSize) {
          dispatch({
            type: SET_ARABIC_TEXT_SIZE,
            payload: arabicTextSize
          });
        }
        let banglaTextSize = JSON.parse(await AsyncStorage.getItem("banglaTextSize"));
        if (banglaTextSize) {
          dispatch({
            type: SET_BANLGA_TEXT_SIZE,
            payload: banglaTextSize
          });
        }
        let englishTextSize = JSON.parse(await AsyncStorage.getItem("englishTextSize"));
        if (englishTextSize) {
          dispatch({
            type: SET_ENGLIS_TEXT_SIZE,
            payload: englishTextSize
          });
        }
        ...
        let isShowBanglaText = JSON.parse(await AsyncStorage.getItem("isShowBanglaText"));
        ...
        let isShowEnglishText = JSON.parse(await AsyncStorage.getItem("isShowEnglishText"));
        ...
        let isShowTafsir = JSON.parse(await AsyncStorage.getItem("isShowTafsir"));
        ...
        let isShowAyahDetails = JSON.parse(await AsyncStorage.getItem("isShowAyahDetails"));
        ...
        let isShowCopyButton = JSON.parse(await AsyncStorage.getItem("isShowCopyButton"));
        ...
        let isShowSaveButton = JSON.parse(await AsyncStorage.getItem("isShowSaveButton"));
        ...
        let favouriteSurahList = JSON.parse(await AsyncStorage.getItem("favouriteSurahList"));
        ...
        let isShowAudioPlayer = JSON.parse(await AsyncStorage.getItem("isShowAudioPlayer"));
        ...
        if (nobleQuran) {
          dispatch({
            ...
            payload: nobleQuran.readLater
          });
          dispatch({
            type: SET_LAST_READ_SURAH,
            payload: nobleQuran.lastRead
          });
          dispatch({
  ...
  return <View className="" style={{
    ...
    justifyContent: "center"
      ...
      <CustomStatusBar />
      <Image className="w-48 h-48" source={require("../../assets/image/brand.png")} />
    </View>;
});
